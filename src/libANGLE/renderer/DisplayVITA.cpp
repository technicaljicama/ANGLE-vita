...
// DisplayVITA.cpp: Android implementation of egl::Display

#include "libANGLE/renderer/DisplayVITA.h"
...
DisplayVITA::DisplayVITA(const egl::DisplayState &state)
    : DisplayEGL(state),
...
DisplayVITA::~DisplayVITA()
{
}

egl::Error DisplayVITA::initialize(egl::Display *display)
{
...
/*
    ...
    mSupportsSurfaceless = false;//mEGL->hasExtension("EGL_KHR_surfaceless_context");
    ...
    printf("VITA DSIP IKNIT\n");
    return egl::NoError();
}

void DisplayVITA::terminate()
{
    DisplayGL::terminate();
/*
...
SurfaceImpl *DisplayVITA::createWindowSurface(const egl::SurfaceState &state,
...
SurfaceImpl *DisplayVITA::createPbufferSurface(const egl::SurfaceState &state,
                                                  const egl::AttributeMap &attribs)
{
    EGLConfig config;
    EGLint numConfig;
...
SurfaceImpl *DisplayVITA::createPbufferFromClientBuffer(const egl::SurfaceState &state,
                                                           EGLenum buftype,
...
SurfaceImpl *DisplayVITA::createPixmapSurface(const egl::SurfaceState &state,
...
ContextImpl *DisplayVITA::createContext(const gl::ContextState &state,
                                           ...
                                           const egl::AttributeMap &attribs)
{
    ...
    else
    {
        ...
        // share context because there are no internal resources in DisplayVITA that are shared
        // at the GL level.
...
template <typename T>
void DisplayVITA::getConfigAttrib(EGLConfig config, EGLint attribute, T *value) const
{
    EGLint tmp;
...
void DisplayVITA::getConfigAttribIfExtension(EGLConfig config,
                                                EGLint attribute,
                                                T *value,
...
egl::ConfigSet DisplayVITA::generateConfigs()
{
...
bool DisplayVITA::testDeviceLost()
{
    return false;
}

egl::Error DisplayVITA::restoreLostDevice(const egl::Display *display)
{
    UNIMPLEMENTED();
    return egl::NoError();
}

bool DisplayVITA::isValidNativeWindow(EGLNativeWindowType window) const
{
    return true;
}

DeviceImpl *DisplayVITA::createDevice()
{
    UNIMPLEMENTED();
    return nullptr;
}

egl::Error DisplayVITA::waitClient(const gl::Context *context)
{
    UNIMPLEMENTED();
    return egl::NoError();
}

egl::Error DisplayVITA::waitNative(const gl::Context *context, EGLint engine)
{
    UNIMPLEMENTED();
    return egl::NoError();
}

egl::Error DisplayVITA::makeCurrent(egl::Surface *drawSurface,
                                       ...
                                       gl::Context *context)
{
    ...
    if (newSurface != currentContext.surface || newContext != currentContext.context)
    {
        ...
        // {
            // return;// egl::Error("eglMakeCurrent failed");
        // }
        currentContext.surface = newSurface;
        currentContext.context = newContext;
    }
#endif
    return egl::NoError();// DisplayGL::makeCurrent(drawSurface, readSurface, context);
}

gl::Version DisplayVITA::getMaxSupportedESVersion() const
{
    return mRenderer->getMaxSupportedESVersion();
}

void DisplayVITA::destroyNativeContext(EGLContext context)
{
...
void DisplayVITA::generateExtensions(egl::DisplayExtensions *outExtensions) const
{
...
egl::Error DisplayVITA::makeCurrentSurfaceless(gl::Context *context)
{
...
egl::Error DisplayVITA::createRenderer(EGLContext shareContext,
...
/*
    ...
    printf("ANGLE: Init functions\n");
    std::unique_ptr<FunctionsGL> functionsGL(mEGL->makeFunctionsGL());
    functionsGL->initialize(mDisplayAttributes);
    printf("ANGLE: Done Init functions\n");
    outRenderer->reset(new RendererEGL(std::move(functionsGL), mDisplayAttributes, this, shareContext));

    CurrentNativeContext &currentContext = mCurrentNativeContext[std::this_thread::get_id()];
    if (makeNewContextCurrent)
    {
        currentContext.surface = mDummyPbuffer;
        currentContext.context = shareContext;
    }
    else
    {
