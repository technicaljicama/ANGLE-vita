...
void GL_APIENTRY AngleInit(EGLDisplay dpy, EGLSurface draw, GLeglContext ctx)
{
    ANGLE_SCOPED_GLOBAL_LOCK();
    
    
    EGLDisplay display     = aeglGetDisplay(EGL_DEFAULT_DISPLAY);//Display::GetDisplayFromNativeDisplay(EGL_DEFAULT_DISPLAY, AttributeMap());
    // Surface *drawSurface = static_cast<Surface *>(draw);
    // Surface *readSurface = static_cast<Surface *>(read);
    gl::Context *sharedGLContext = static_cast<gl::Context *>(ctx);
    egl::Thread *thread = egl::GetCurrentThread();
    // egl::MakeCurrent(dpy, draw, read, ctx)
    // egl::SetContextCurrent(thread, (gl::Context*)ctx);
    // initProcsGLES(Version(2,0), extensionSet);
    // std::unique_ptr<FunctionsGL> functionsGL(nullptr);
    // SetContextCurrent(thread, (gl::Context*)ctx);
    // gl::rx::ContextGL ctxgl(state, nullptr);
    // egl::Display *display             = static_cast<Display *>(dpy);
    // egl::Config *configuration        = static_cast<egl::Config *>(config);
    // gl::Context *sharedGLContext = static_cast<gl::Context *>(share_context);
    // egl::AttributeMap attributes      = egl::AttributeMap::CreateFromIntArray(attrib_list);
    ...
    EGL_GREEN_SIZE, 8,
    EGL_BLUE_SIZE, 8,
    EGL_ALPHA_SIZE, 8,  // If you want an alpha channel
    EGL_BUFFER_SIZE, 32,
    EGL_COLOR_BUFFER_TYPE, EGL_RGB_BUFFER,
    EGL_NONE
};
        aeglInitialize(display, NULL, NULL);
      EGLBoolean c = aeglChooseConfig(display, cfg_attribs, &config, 1, &gnumconfig);
      EGLSurface surf = aeglCreateWindowSurface(display, config, NULL, NULL);
    ctx = aeglCreateContext(display, config, NULL, cfg_attribs);
    // context->makeCurrent((egl::Display*)dpy, (egl::Surface*)draw);
    aeglMakeCurrent(display, surf, surf, ctx);
    // display->createContext(configuration, sharedGLContext, attributes, &context);
    // 
